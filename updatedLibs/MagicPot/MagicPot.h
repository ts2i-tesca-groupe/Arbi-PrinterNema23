...
class MagicPot
{
	public:
		...
		MagicPot(uint8_t pin, uint16_t minRead = 0, unsigned long maxRead = MAX_RAW_VALUE_DEFAULT, uint16_t maxRawRead = MAX_RAW_VALUE_DEFAULT): pin(pin), minRead(minRead), maxRead(maxRead), maxRawRead(maxRawRead) {};
		void begin();
